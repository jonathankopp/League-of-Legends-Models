...
df      = pd.read_csv('LCK_Data/preSweep.csv',index_col='date',parse_dates = True)
prepTeamDat = []
prepPlayerDat = []
rpm = []
players = dict()
...
for i,row in df.iterrows():
    ...
    if player not in players.keys():
        playerData[player] = pd.DataFrame(data=[np.array([0 for i in range(39)])],columns=(df.columns)[5:])
    ...
    playerData[player].loc[len(playerData[player])] = comp
    wma = playerData[player].copy()
    # moving average of all data
    comp_2 = list(wma.ewm(alpha=0.4).mean().loc[len(wma)-1])
    fullTeam_2 = list(comp_2[2:12]) + list(comp_2[32:])
    ...
    sPlayer[player] = np.add(np.array(list(row[5:7])+list(comp_2[17:37])),sPlayer[player])/2.0
    playerC[player] += 1
    if(counter%5==0):
